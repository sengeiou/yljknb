...
<body>
						...
						<div id="sp2" class="mini-splitter"
							...
							<div size="50%" showCollapseButton="false">
								...
								<div id="datagrid1" class="mini-datagrid"
									...
									url="${pageContext.request.contextPath}/k5/f10507001/queryMedDocument.action">
							...
							<div size="50%" showCollapseButton="false">
								<div id="toolbar1" class="mini-toolbar" style="height: 10%;">
									<table style="width:100%;">
										<tr>
											<td style="width:100%;"><input id="baz003s"
												name="baz003s" class="mini-hidden" value="" /> <span
												class="btnMini" style="margin:0 10px;"><em> <input
														type="button" onclick="plcl('5');" value="复议申请" />
												</em> </span></td>
										</tr>
									</table>
								</div>
								...
								<div id="datagrid2" class="mini-datagrid" style="height: 86%;"
									...
									url="${pageContext.request.contextPath}/k5/f10507001/queryMedDocumentDetail.action">
									<div property="columns">
										<div type="checkcolumn"></div>
										...
										<div field="aaa133" width="100" headerAlign="center">扣款理由</div>
										<div field="baz041" width="100" headerAlign="center"
											visible="false">拟扣除金额</div>
										<div field="baz042" width="100" headerAlign="center">已扣除金额</div>
	...
	<script type="text/javascript">
		...
		function onShowRowDetail2(e) {
			...
			$
					.ajax({
						url : "${pageContext.request.contextPath}/k5/f10507001/queryMedDocumentDetailIllegal.action",
						type : "post",
						data : {
							baz001 : row.baz001,
							baz003 : row.baz003
						...
						success : function(text) {
							if (text.success) {
								...
								htmlStr += "<tr><td style='width:100px'>复议申请说明：</td><td colspan='3' style='width:500px'><textarea  id='baz021s' name='baz021'  class='mini-textarea' style='width:400px' ></textarea></td></tr>";
								//htmlStr += "<tr><td>上传附件材料：</td><td colspan='4'><div style='padding-top:4px;'><div class='textareaFile'><input id='fileupload1' class='mini-fileupload' name='filedata' limitType='*' flashUrl='${pageContext.request.contextPath}/resource/scripts/swfupload.swf' uploadUrl='*' onuploadsuccess='onUploadSuccess' onuploaderror='onUploadError' onfileselect='onFileSelect' /></div></div></td></tr>";
								htmlStr += "<tr><td></td></tr>";
								htmlStr += "<tr><td></td><td colspan='2'><span class='btnMini' style='margin:0 10px'><em><input type='button' onclick=preCheck('5') value='提交申请' /></em></span></td><td></td></tr>";
								htmlStr += "</table>";
								htmlStr += "</div>";
		...
		function preCheck(baz102) {
			...
			$
					.ajax({
						url : "${pageContext.request.contextPath}/k5/f10507001/preCheckSingle.action",
						type : "post",
						data : {
							ids : id,
							baz021 : baz021,
		...
		function plcl(baz102) {
			...
			if (baz102 == 5) {
				mini
						.open({
							url : "${pageContext.request.contextPath}/pages/k5/f10507001/f10507001_batchAdd.jsp",
							title : '复议申请',
							width : 350,
							height : 220,
							onload : function() {
		...
		function showDetailHos() {
			...
			$
					.ajax({
						url : "${pageContext.request.contextPath}/k5/f10507002/queryMedDocumentByHos.action",
						type : "post",
						data : {
						...
						success : function(text) {
							if (text.success) {
								...
								for ( var i = 0; i < text.result.length; i++) {
									...
									htmlStr += "<td style='padding-bottom:6px;'>待复议申请数：</td>";
