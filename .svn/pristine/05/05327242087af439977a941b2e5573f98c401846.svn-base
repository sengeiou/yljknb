			...
			for (int i = 0; i < colCount; ++i) {
				...
				if (setter == null) {
					if (log.isDebugEnabled()) {
						log.debug(String.format("not setter of property %s",
								colName));
					}
					continue;
					// throw new RowMapperException("004", String.format("not
					// setter of property %s", colName), null);
				}
				if (getter == null) {
					if (log.isDebugEnabled()) {
						log.debug(String.format("not getter of property %s",
								colName));
					}
					continue;
					// throw new RowMapperException("004", String.format("not
					// getter of property %s", colName), null);
				}
				...
				try {
					...
					throw new RowMapperException(String.format(
							"invoke error[method:%s]", setter.getName()), e);
				}
			}

		} catch (SQLException se) {
			throw new RowMapperException(String.format("SQL异常:%s", se
					.getMessage()), se);
		}
