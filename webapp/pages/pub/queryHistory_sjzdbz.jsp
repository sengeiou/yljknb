...
<body style="background-color:#F3F3F3 ;">
	...
	<script type="text/javascript">
		...
		//tab页1的js查询
		function valuechanged1(data) {
			var yishisjz = document.getElementById('yishisjz');
			$
					.ajax({
						url : "${pageContext.request.contextPath}/common/queryHistoryDBZ.action",
						type : "post",
						data : {
							baz011 : data.baz011,
							flag : data.flag
		...
		function filedownload(fileurl) {
			$
					.ajax({
						url :"${pageContext.request.contextPath}/common/fileExistDBZ.action",
						type : "post",
						data : {
							baz014 : fileurl
						},
						success : function(text) {
							if(text.success){
								location.href="${pageContext.request.contextPath}/common/downLoadDBZ.action?baz014="+fileurl;
							}else{
								mini.alert('文件不存在');
							}
