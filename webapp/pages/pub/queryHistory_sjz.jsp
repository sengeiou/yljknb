...
<body style="background-color:#F3F3F3 ;">
	...
	<script type="text/javascript">
		...
		function valuechanged1(data) {
			...
			$
					.ajax({
						url : "${pageContext.request.contextPath}/common/queryHistory.action",
						type : "post",
						data : {
							baz003 : data.baz003,
							flag : data.flag
		...
		function filedownload(fileurl) {
			$
					.ajax({
						url :"${pageContext.request.contextPath}/common/fileExist.action",
						type : "post",
						data : {
							baz010 : fileurl
						},
						success : function(text) {
							if(text.success){
								location.href="${pageContext.request.contextPath}/common/downLoad.action?baz010="+fileurl;
							}else{
								mini.alert('文件不存在');
							}
