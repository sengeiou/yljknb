...
<body>
						...
						<div id="sp2" class="mini-splitter"
							...
							<div size="38%" showCollapseButton="false">
								...
								<div id="datagrid1" class="mini-datagrid" sortMode="client"
									...
									url="${pageContext.request.contextPath}/k2/f10204001/queryMedDocument.action">
							...
							<div size="62%" showCollapseButton="false">
										...
										<tr>
											<td style="width:100%;">
												...
												<!--menu--> <span class="btnMini" style="margin:0 10px;"><em>
														<input type="button" value="复议通过 " onclick="plcl('2');">
												</em> </span> <span class="btnMini" style="margin:0 10px;"><em>
														<input type="button" value="复议扣除" onclick="plcl('3');">
												</em> </span> <input id="baz003s" name="baz003s" class="mini-hidden"
												value="" /> <span class="btnMini" style="margin:0 10px;"><em>
														<input type="button" value="复议部分扣除" onclick="plcl('4');">
												</em> </span> <input id="baz003s" name="baz003s" class="mini-hidden"
												value="" />
											</td>
										</tr>
								...
								<div id="datagrid2" class="mini-datagrid" style="height: 86%;"
									...
									showSummaryRow="true"
									url="${pageContext.request.contextPath}/k2/f10204001/queryMedDocumentDetail.action">
									<div property="columns">
										<div type="checkcolumn"></div>
	...
	<script type="text/javascript">
		...
		function onShowRowDetail2(e) {
			...
			$
					.ajax({
						url : "${pageContext.request.contextPath}/k2/f10204001/queryMedDocumentDetailIllegal.action",
						type : "post",
						data : {
							baz001 : row.baz001,
							baz003 : row.baz003
						...
						success : function(text) {
							if (text.success) {
								...
								htmlStr += "<tr><td></td></tr>";
								htmlStr += "<tr><td></td><td colspan='2'><span class='btnMini' style='margin:0 10px'><em><input type='button' onclick=preCheck('2') value='复议通过' /></em></span><span class='btnMini' style='margin:0 10px'><em><input type='button' onclick=preCheck('3') value='复议不通过' /></em></span></td><td></td></tr>";
								htmlStr += "</table>";
								htmlStr += "</div>";
		...
		function preCheck(baz102) {
			...
			$
					.ajax({
						url : "${pageContext.request.contextPath}/k2/f10204001/preCheckSingle.action",
						type : "post",
						data : {
							ids : id,
							baz021 : baz021,
		...
		function plcl(baz102) {
			...
			for ( var int = 0; int < jsongrid2.length; int++) {
				if (kcje > 0) {
					if (kcje > jsongrid2[int].baz042) {
						kcje = jsongrid2[int].baz042;
					}
				} else {
					kcje = jsongrid2[int].baz042;
				}
			...
			if (baz102 == 2) {
				mini
						.open({
							url : "${pageContext.request.contextPath}/pages/k2/f10204001/f10204001_plcl.jsp",
							title : '复议通过',
							width : 350,
							height : 200,
							onload : function() {
								...
								iframe.contentWindow.SetData(date);
