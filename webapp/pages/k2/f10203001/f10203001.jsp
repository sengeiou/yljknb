...
<body>
				...
				<div id="sp1" class="mini-splitter" style="width:100%;height:100%;"
					...
					<div size="14%" showCollapseButton="true">
						<div id="editForm1" class="cxTabbox">
							<form id="form1" action="#" onsubmit="return flase">
									...
									<tr>
										...
										<td>
											<div class="inputL" style="width: 100px">
												<div class="inputR">
													<div class="inputC">
														<input id="aka130" name="aka130" class="mini-combobox" multiSelect="true"
						...
						<div id="sp2" class="mini-splitter"
							...
							<div size="50%" showCollapseButton="false">
								...
								<div id="datagrid1" class="mini-datagrid"
									...
									url="${pageContext.request.contextPath}/k2/f10203001/queryMedDocument.action">
							...
							<div size="50%" showCollapseButton="false">
										...
										<tr>
											<td style="width:100%;"><span class="btnMini"
												style="margin:0 10px;"><em><input type="button"
														onclick="plcl('01')" value="正常" />
												</em>
											</span> <span class="btnMini" style="margin:0 10px;"><em><input
														type="button" onclick="plcl('04')" value="违规" />
												</em>
											</span> <input id="baz003s" name="baz003s" class="mini-hidden"
												value="" /></td>
										</tr>
									</table>
								</div>
								<div id="datagrid2" class="mini-datagrid" style="height: 86%;"
									...
									url="${pageContext.request.contextPath}/k2/f10203001/queryMedDocumentDetail.action">
									<div property="columns">
										<div type="checkcolumn"></div>
										<div type="expandcolumn">~</div>
										<div field="baz031" type="comboboxcolumn" width="100"
				...
				<div id="audit_tabs" class="mini-tabs" activeIndex="0"
					...
					<div title="医院汇总">
						<div style="text-align:center; margin:10px;">
							<form id="form2" action="#" onsubmit="return flase">
	...
	<script type="text/javascript">
		...
		function onShowRowDetail2(e) {
			...
			$
					.ajax({
						url : "${pageContext.request.contextPath}/k2/f10203001/queryMedDocumentDetailIllegal.action",
						type : "post",
						data : {
							baz001 : row.baz001,
							baz003 : row.baz003
						...
						success : function(text) {
							if (text.success) {
								...
								for ( var i = 0; i < text.result[0].kc72List.length; i++) {
									htmlStr += "<tr><td style='width:100px'>违规描述：</td><td colspan='3' style='width:500px'>"
								...
								htmlStr += "<tr><td>扣除金额：</td><td colspan='3'><input name='baz041s' id='baz041s' type='text'  /></td></tr>";
								htmlStr += "<tr><td></td><td colspan='2'><span class='btnMini' style='margin:0 10px;'><em><input type='button' onclick=singleCheck('01') value='正常' /></em></span><span class='btnMini' style='margin:0 10px'><em><input type='button' onclick=singleCheck('04')  value='待申诉' /></em></span></td><td></td></tr>";
		...
		function singleCheck(baz031) {
			...
			if (record.length == 1) {
				...
				$
						.ajax({
							url : "${pageContext.request.contextPath}/k2/f10203001/medDocumentDetailOperateSingle.action",
							type : "post",
							data : {
								baz021 : baz021,
								baz031 : baz031,
		...
		function plcl(baz031) {
			...
			if (baz031 == 01) {
				mini
						.open({
							url : "${pageContext.request.contextPath}/pages/k2/f10203001/f10203001_plcl.jsp",
							title : '正常',
							width : 350,
							height : 200,
							onload : function() {
		...
		function showDetailHos() {
			...
			$
					.ajax({
						url : "${pageContext.request.contextPath}/k2/f10203001/queryMedDocumentByHos.action",
						type : "post",
						data : {
							query : akb020orakb021
						},
						success : function(text) {
							if (text.success) {
								$("#menuList1").empty();
								for ( var i = 0; i < text.result.length; i++) {
									...
									htmlStr += "<a class='top_name_fontsize' href='#' onclick=setandsearch('"
											+ text.result[i].akb021
											+ "','"
											+ text.result[i].akb020.trim()
											+ "')>"
											+ text.result[i].akb021
											+ "</a>";
									htmlStr += "</div>";
									...
									htmlStr += "<td style='padding-bottom:6px;'>待人工初审数：</td>";
									htmlStr += "<td style='padding-bottom:6px;'>超期数：</td>";
									htmlStr += "</tr>";
									...
									htmlStr += "<td style='padding-bottom:6px;'><div class='td_number'>"
											+ text.result[i].drgcs
											+ "</div>	</td>";
									htmlStr += "<td style='padding-bottom:6px;'><div class='td_number2'>"
											+ text.result[i].cqs
											+ "</div>	</td>";
									htmlStr += "</tr>";
		...
		function showDetailGz() {
			...
			$("#menuList2")
					.html("<font color=\"red\">正在查询规则汇总数据,请稍等...</font>");
			var data = {
				baz031 : '05',
				baz021 : '3',
				baz101 : '',
					...
					function(data) {
						if (data.success) {
							...
							for ( var i = 0; i < data.result.length; i++) {
								var htmlStr = "";
								htmlStr += "<div class=\"top_name\"><a class=\"top_name_fontsize\" href=\"#\" onclick=\"findByGz('"
										+ data.result[i].gzflbm
										+ "','"
										+ data.result[i].gzbm
										+ "')\">"
										+ data.result[i].gzmc + "</a></div>";
								htmlStr += "<table cellpadding=\"0\" class=\"cxTab\" cellspacing=\"0\" border=\"0\" width=\"100%\">";
								htmlStr += "<tr><td style=\"padding-bottom:6px;\">待人工初审数：</td><td style=\"padding-bottom:6px;\">超期数：</td></tr>";
								htmlStr += "<tr><td style=\"padding-bottom:6px;\" rowspan=\"3\"><div class=\"td_number \">"
										+ data.result[i].drgcs
										+ "</div></td><td style=\"padding-bottom:6px;\" rowspan=\"3\"><div class=\"td_number2 \">"
										+ data.result[i].cqs
										+ "</div></td></tr>";
								htmlStr += "</table>";
							...
							$("#menuList2")
									.html(
											"<font color=\"red\">没有找到相关规则数据<br/>请重新选择</font>");
						}
					});
		}
