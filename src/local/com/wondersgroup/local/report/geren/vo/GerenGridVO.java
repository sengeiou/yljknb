package com.wondersgroup.local.report.geren.vo;

import java.util.Date;

import com.wondersgroup.bc.auditadministrativedisposal.admindis.model.dto.Kc21DTO;

public class GerenGridVO extends Kc21DTO{

	private String id;//参保人id
	
	private String jzlx;//就诊类型
	
	private String fylb;//费用类别
	
	private String tjsj1;//统计开始时间
	
	private String tjsj2;//统计结束时间
	
	private String yymc;//医院名称
	
	private String bz;//病种
	
	private String ys;//医师
	
	private String jzcs;//就诊次数
	
	private double fsje;//发生金额
	
	private String sjkd;//时间跨度

	private String xmmc;//项目名称

	private String cfts;//处方天数
	
	private String rlts;//日历天数
	
	private String clts;//超量天数
	
	private String sl;//数量
	
	private Date scnysj;//首次拿药时间
	
	private Date mcnysj;//首次拿药时间
	
	private Date fyfssj;//费用发生时间
	
	private String cbrbm;//参保人编码
	
	private String xm;//姓名
	
	private String sfz;//身份证
	
	private String xb;//性别
	
	private String age;//年龄
	
	private String jzyy;//就诊医院
	
	private double fyje;//费用金额
	
	private String wgyy;//违规原因
	
	private String wglx;//违规类型
	
	private String cfjg;//处罚结果
	
	private double cfje;//处罚金额
	
	private String jbr;//经办人
	
	private Date jbrq;//经办日期
	
	private String clyj;//处理意见
	
	public String getYymc() {
		return yymc;
	}

	public void setYymc(String yymc) {
		this.yymc = yymc;
	}

	public String getBz() {
		return bz;
	}

	public void setBz(String bz) {
		this.bz = bz;
	}

	public String getYs() {
		return ys;
	}

	public void setYs(String ys) {
		this.ys = ys;
	}

	public String getJzcs() {
		return jzcs;
	}

	public void setJzcs(String jzcs) {
		this.jzcs = jzcs;
	}

	public String getSjkd() {
		return sjkd;
	}

	public void setSjkd(String sjkd) {
		this.sjkd = sjkd;
	}

	public String getId() {
		return id;
	}

	public void setId(String id) {
		this.id = id;
	}

	public String getJzlx() {
		return jzlx;
	}

	public void setJzlx(String jzlx) {
		this.jzlx = jzlx;
	}

	public String getTjsj1() {
		return tjsj1;
	}

	public void setTjsj1(String tjsj1) {
		this.tjsj1 = tjsj1;
	}

	public String getTjsj2() {
		return tjsj2;
	}

	public void setTjsj2(String tjsj2) {
		this.tjsj2 = tjsj2;
	}

	public String getFylb() {
		return fylb;
	}

	public void setFylb(String fylb) {
		this.fylb = fylb;
	}

	public double getFsje() {
		return fsje;
	}

	public void setFsje(double fsje) {
		this.fsje = fsje;
	}

	public String getXmmc() {
		return xmmc;
	}

	public void setXmmc(String xmmc) {
		this.xmmc = xmmc;
	}

	public String getCfts() {
		return cfts;
	}

	public void setCfts(String cfts) {
		this.cfts = cfts;
	}

	public String getRlts() {
		return rlts;
	}

	public void setRlts(String rlts) {
		this.rlts = rlts;
	}

	public String getClts() {
		return clts;
	}

	public void setClts(String clts) {
		this.clts = clts;
	}

	public String getSl() {
		return sl;
	}

	public void setSl(String sl) {
		this.sl = sl;
	}

	public Date getScnysj() {
		return scnysj;
	}

	public void setScnysj(Date scnysj) {
		this.scnysj = scnysj;
	}

	public Date getMcnysj() {
		return mcnysj;
	}

	public void setMcnysj(Date mcnysj) {
		this.mcnysj = mcnysj;
	}

	public Date getFyfssj() {
		return fyfssj;
	}

	public void setFyfssj(Date fyfssj) {
		this.fyfssj = fyfssj;
	}

	public String getXm() {
		return xm;
	}

	public void setXm(String xm) {
		this.xm = xm;
	}

	public String getSfz() {
		return sfz;
	}

	public void setSfz(String sfz) {
		this.sfz = sfz;
	}

	public String getXb() {
		return xb;
	}

	public void setXb(String xb) {
		this.xb = xb;
	}

	public String getAge() {
		return age;
	}

	public void setAge(String age) {
		this.age = age;
	}

	public String getJzyy() {
		return jzyy;
	}

	public void setJzyy(String jzyy) {
		this.jzyy = jzyy;
	}

	public String getWgyy() {
		return wgyy;
	}

	public void setWgyy(String wgyy) {
		this.wgyy = wgyy;
	}

	public String getWglx() {
		return wglx;
	}

	public void setWglx(String wglx) {
		this.wglx = wglx;
	}

	public String getCbrbm() {
		return cbrbm;
	}

	public void setCbrbm(String cbrbm) {
		this.cbrbm = cbrbm;
	}

	public String getCfjg() {
		return cfjg;
	}

	public void setCfjg(String cfjg) {
		this.cfjg = cfjg;
	}

	public String getJbr() {
		return jbr;
	}

	public void setJbr(String jbr) {
		this.jbr = jbr;
	}

	public Date getJbrq() {
		return jbrq;
	}

	public void setJbrq(Date jbrq) {
		this.jbrq = jbrq;
	}

	public String getClyj() {
		return clyj;
	}

	public void setClyj(String clyj) {
		this.clyj = clyj;
	}

	public double getFyje() {
		return fyje;
	}

	public void setFyje(double fyje) {
		this.fyje = fyje;
	}

	public double getCfje() {
		return cfje;
	}

	public void setCfje(double cfje) {
		this.cfje = cfje;
	}
	
}
