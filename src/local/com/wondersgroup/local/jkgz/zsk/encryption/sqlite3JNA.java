...
public class sqlite3JNA {
	...
	public interface Clibrary extends Library{  
		//加载链接库  
		Clibrary INSTANTCE = (Clibrary) Native.loadLibrary("sqlite3", Clibrary.class);
		  
		//链接库中的方法  
		int sqlite3_open(String filename, PointerByReference ppDb);  
		int sqlite3_rekey(Pointer pnt , String key , int length);
		int sqlite3_key(Pointer pnt , String key , int length);
		int sqlite3_close(Pointer pnt );
		int sqlite3_free(PointerByReference errMsg);
		public static interface sqlite3_callback extends Callback{
			int callback(Pointer data, int columnCount, PointerByReference values, PointerByReference names);
		}
		int sqlite3_exec(Pointer pDb, String cmd,   sqlite3_callback callback, Pointer udp, PointerByReference errMsg);
	} 
